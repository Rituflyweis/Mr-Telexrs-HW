...
exports.getAllPatients = async (query = {}) => {
  ...
  const patientsWithStats = await Promise.all(
    patients.map(async (patient) => {
      ...
      const intakeForm = await IntakeForm.findOne({ patient: patientId, status: "submitted" }).sort({ createdAt: -1 })
        .lean();

      const consent = intakeForm;
...
exports.updatePatientDoctor = async (patientId, doctorId) => {
  const patient = await Patient.findById(patientId);
  if (!patient) {
    throw new AppError('Patient not found', 404);
  }
  // Also update user status
  await User.findByIdAndUpdate(patient.user, { doctorId });

  return await Patient.findById(patientId)
    .populate({
      path: 'user',
      select: 'firstName lastName email phoneNumber countryCode isActive isVerified doctorId'
    })
    .lean();
};
