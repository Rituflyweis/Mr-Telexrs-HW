...
exports.getRefills = async (userId, query) => {
  const patient = await getPatient(userId);
  console.log('getRefills', userId, query, "patient======",patient._id );
...
exports.deleteRefill = async (userId, refillId) => {
  const patient = await getPatient(userId);
  const refill = await Refill.findOneAndDelete({ _id: refillId, patient: patient._id });
  if (!refill) {
    throw new AppError('Refill not found', 404);
  }
...
exports.createOrderFromRefill = async (userId, orderData) => {
  ...
  const totalAmount = subtotal + shippingCharges + tax + consultantFees - discount;
