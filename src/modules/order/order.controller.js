...
exports.getOrders = async (req, res, next) => {
  try {
    const result = await orderService.getPatientOrders(req.user.id, req.query);
    res.status(200).json({ 
      success: true, 
      data: result.orders,
...
exports.getOrdersForDoctor = async (req, res, next) => {
  try {
    const result = await orderService.getOrdersForDoctor(req.user.id, req.query);
    res.status(200).json({ 
      success: true, 
      data: result.orders,
      pagination: result.pagination,
...
exports.createOrderByDoctor = async (req, res, next) => {
  try {
    const order = await orderService.createOrderByDoctor(req.user.id, req.body.orderId);
    res.status(201).json({ success: true, message: 'Doctor Ordered created successfully', data: order });
  } catch (err) { next(err); }
};

// Create order with new prescription
exports.createPrescriptionOrderByDoctor = async (req, res, next) => {
  try {
    const order = await orderService.createPrescriptionOrderByDoctor(req.user.id, req.body);
    res.status(201).json({ success: true, message: 'Doctor Ordered with Prescription created successfully', data: order });
  } catch (err) { next(err); }
};
